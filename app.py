...
app.config['MAIL_DEBUG'] = True  # Enable SMTP debugging
...
# Add these new routes at the bottom of your route definitions (before error handlers)
@app.route('/custom-login', methods=['GET', 'POST'])
def custom_login():
    # Call original login view but handle redirection ourselves
    response = user_manager.login_view()
...
@app.route('/custom-register', methods=['GET', 'POST'])
def custom_register():
    response = user_manager.register_view()
